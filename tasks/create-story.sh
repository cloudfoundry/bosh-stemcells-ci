...
tasks="[]"
if [[ ${DESCRIPTION} == *"security"* ]]; then
  tasks='[ { "description": "Validate the packages for the USN referenced in #bosh-private CFF Slack are included in the Stemcell" } ]'
fi
...
curl --show-error --fail -X POST "https://www.pivotaltracker.com/services/v5/projects/$PROJECT_ID/stories" \
