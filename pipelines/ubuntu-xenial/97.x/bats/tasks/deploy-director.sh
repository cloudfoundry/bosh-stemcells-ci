...
bosh-cli interpolate bosh-deployment/bosh.yml \
  ...
  -o bosh-stemcells-ci/pipelines/ubuntu-xenial/97.x/bats/ops/remove-health-monitor.yml \
  -o bosh-stemcells-ci/pipelines/ubuntu-xenial/97.x/bats/ops/use-candidate-stemcell.yml \
  -v dns_recursor_ip=8.8.8.8 \
  -v director_name=bats-director \
  -v local_candidate_stemcell=$(realpath stemcell/*.tgz) \
  --vars-file <( bosh-stemcells-ci/pipelines/ubuntu-xenial/97.x/bats/iaas/$BAT_INFRASTRUCTURE/director-vars ) \
  > director.yml
