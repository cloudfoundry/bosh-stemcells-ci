...
bosh-cli interpolate bosh-deployment/bosh.yml \
  ...
  -o bosh-stemcells-ci/pipelines/ubuntu-xenial/250.x/bats/ops/remove-health-monitor.yml \
  -v dns_recursor_ip=8.8.8.8 \
  -v director_name=bats-director \
  --vars-file <( bosh-stemcells-ci/pipelines/ubuntu-xenial/250.x/bats/iaas/$BAT_INFRASTRUCTURE/director-vars ) \
  > director.yml
